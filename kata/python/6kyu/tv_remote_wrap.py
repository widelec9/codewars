...
def tv_remote(word):
    ...
    curr_mode = 1
    for c in word:
        if c.islower() or (c in kb and not ((curr_mode == 2 and c in kb) or (curr_mode == 3 and c in kb_sym))):
            new_mode = 1
        elif c.isupper() or (curr_mode == 2 and c in kb):
            new_mode = 2
        else:
            new_mode = 3
...
def get_dist_new_pos(curr_pos, new_c, curr_dist, curr_mode, new_mode):
    ...
    curr_dist += min((keyb.shape[1] - abs(new_pos[1] - curr_pos[1])) % keyb.shape[1], abs(new_pos[1] - curr_pos[1]))
    if new_c == 'aA#':
